...
class LeapListenser:
    ...
    def _receive_allegro(self, pose):
        pose = lhu.amerigo(pose.position, zeros=False)
        self.prev_pos = self.curr_pos
        self.curr_pos = np.array(pose)
        self.dxl_client.write_desired_pos(self.motors, self.curr_pos)
...
class LeapPublisher:
    def __init__(self, motor):
        rospy.init_node("LeapPublisher")

        self.dxl_client = motor.dxl_client
        self.motors = motor.motors
        
        self.joint_state_publisher = rospy.Publisher('/leaphand_node/joint_states', JointState, queue_size = 1)
    ...
    def publish_state(self):
        ...
        joint_velocity = self.dxl_client.read_vel()
        joint_effort = self.dxl_client.read_cur()

        data = JointState()
        data.position = joint_position
        data.velocity = joint_velocity
        data.effort = joint_effort
...
def main(**kwargs):
    ...
    leap_listener_process = Process(target=leap_listener, args=(motor,))
    leap_publisher_process = Process(target=leap_publisher, args=(motor,))

    leap_listener_process.start()
    time.sleep(1)
    leap_publisher_process.start()

    time.sleep(1)

    leap_listener_process.join()
    time.sleep(1)
    leap_publisher_process.join()
    

def leap_listener(motor):
    LeapListenser(motor)

def leap_publisher(motor):
    LeapPublisher(motor)
